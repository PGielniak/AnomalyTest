...
namespace AnomalyTest
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            string datasource = @"<BLOB_FILE_SAS_URL_FILE_NOT_CONTAINER>";
            ...
            try
            {
                //You can exchange this line with model_id_raw = new Guid("<GUID OF THE MODEL>");
                // after first run to save time. You can copy the guid from the console
                model_id_raw = await trainAsync(client,datasource,start_time,end_time);
                ...
                {
                    counter++;
                    if (item.Value.IsAnomaly == true)
                    {
                        Console.WriteLine("Anomaly Found!");
                        Console.WriteLine($"Record Number {counter}");
                        Console.WriteLine($"Time: ");
                        Console.WriteLine($"{item.Timestamp}");
                        Console.WriteLine("Contributors");
                        foreach (var contributor in item.Value.Contributors)
                        {
                            Console.WriteLine(contributor.Variable);
                        }
                    }
                };
                if (result != null)
        ...
        private static async Task<Guid> trainAsync(AnomalyDetectorClient client, string datasource, DateTimeOffset start_time, DateTimeOffset end_time, int max_tryout = 500)
        {
            try
            {
        ...
        private static async Task<DetectionResult> detectAsync(AnomalyDetectorClient client, string datasource, Guid model_id, DateTimeOffset start_time, DateTimeOffset end_time, int max_tryout = 500)
        {
            try
            {
        ...
        private static async Task deleteAsync(AnomalyDetectorClient client, Guid model_id)
        {
        ...
        private static async Task exportAsync(AnomalyDetectorClient client, Guid model_id, string model_path = "model.zip")
        {
            try
            {
